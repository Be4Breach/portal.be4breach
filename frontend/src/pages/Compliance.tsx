...
const Compliance = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as { answers?: Record<string, AnswerRecord>; complete?: boolean };
  ...
  // Persist answers + completion
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ answers, complete: questionnaireComplete }));
  }, [answers, questionnaireComplete]);
    ...
    return (
      <div className="space-y-6 w-full max-w-6xl animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <p className="text-xs text-muted-foreground">Evidence uploads happen inside the dashboard after you submit answers.</p>
          <Button
